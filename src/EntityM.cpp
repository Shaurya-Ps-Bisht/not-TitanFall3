...
EntityM::EntityM(const std::string &name, glm::vec3 &initialPosition, glm::vec3 &initialScale,
                 const Shader *initialShader,
                 const char *modelPath)
    : Entity(name, initialPosition, initialScale, initialShader), m_model( new Model(modelPath, false)), hasAnimation(false)
{
}
...
EntityM::EntityM(const std::string &name, glm::vec3 &initialPosition, glm::vec3 &initialScale,
    ...
    : Entity(name, initialPosition, initialScale, initialShader), m_model(new Model(modelPath, true)),
...
EntityM::EntityM(const std::string &name, const char *texturePath, glm::vec3 &initialPosition, glm::vec3 &initialScale,
    ...
    : Entity(name, initialPosition, initialScale, initialShader), m_model(new Model(modelPath, false)), m_animation(),
...
EntityM::EntityM(const std::string &name, const char *texturePath, glm::vec3 &initialPosition, glm::vec3 &initialScale,
    ...
    : Entity(name, initialPosition, initialScale, initialShader), m_model(new Model(modelPath, modelMatrices, amount, false)),
                   ...
                   std::vector<lightPoint> &lightPoints, glm::mat4 lightSpaceMatrix)
{
...

    {
        ...
        if (hasExplitcitTexture) explicitTexture.Bind();        
        if (instanced)
        {
